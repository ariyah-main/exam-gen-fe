...
function format_prompt(cloud_exam, domain, task_statement, type) {
	...
	let skills_list = format_skills(task_statement.skills);
	let questions_list = format_training_questions(task_statement.training_questions);
...
function format_training_questions(training_questions) {
	training_questions.forEach((q, i) => {
		skills_list += `${i + 1}.` + q + ' ';
	});
}
